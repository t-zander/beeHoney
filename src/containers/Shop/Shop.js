...
class Shop extends Component {
  ...
  click() {
    console.log('click from nav link!!!');
  }
  render() {
      ...
      <div className="shop">
        <div className="shop__mainContent">
          <div className="shop__chooseCateg">
            ...
            <div className="shop__nav">
              {categories ? categories.map(category =>
                <NavLink key={category._id} to={`/shop/${category._id}`} activeClassName="shop__active">
                  {category.name}
                </NavLink>)
              : null}
            </div>
          ...
          <div className="shop__arrow" >
            <Link activeClass="active" to="products" spy={true} smooth={true} offset={50} duration={500} onSetActive={this.handleSetActive}>
              <img src={arrowDown} />
            </Link>
          </div>
        </div>
        ...
        <Route path="/shop/:categoryId" component={ProductsLayout} test={'test'}/>
